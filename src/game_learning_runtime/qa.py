...
def run_qa(
    ...
    *,
    ...
    checks = tuple(
        _run_check(name, command, Path(project) if project else None, timeout)
        for name, command in commands
    )
    finished = datetime.now(timezone.utc)
    result = QAResult(
        QA_SCHEMA_VERSION, goal, now.isoformat(), finished.isoformat(), str(run_dir), checks
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run bounded GLR QA checks and build an HTML report"
    )
    ...
    result = run_qa(
        args.goal, output_root=args.output_root, project=args.project, commands=commands
    )
