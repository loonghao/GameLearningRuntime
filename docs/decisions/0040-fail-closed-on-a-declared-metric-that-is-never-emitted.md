...
- Three counters are recorded as first-class run metrics —
  `declared_metrics`, `emitted_metrics`, `missing_metrics_count` — and projected
  in `glr.cli-output.v1` through `runs show`. A scheduler reads all three
  without walking events or parsing a log. The third is a count, not the name
  list: `missing_metrics` inside the same envelope is already the names.
- `SyncCollector` takes `store` and `run_id` and binds the ledger it builds, so
  the run store can count emissions into it. A declaration without a binding is
  inert by design — the counters are derived from what the store counted, and a
  ledger only the collector can see would report a gap on every episode.
