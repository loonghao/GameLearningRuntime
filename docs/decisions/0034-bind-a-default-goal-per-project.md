...
start. `goal show` reports `source_status` (`unchanged`, `changed`, `missing`)
and `context_status` (`unbound`, `bound`, `unresolved`), and a deleted default
goal is reported as a gap that asks for a rebind rather than as a silent
failure.
...
goal file is copied into the run directory as an auditable receipt.
...
- A SHA-256 of the goal file makes a drifted or deleted default visible before
  training starts.
