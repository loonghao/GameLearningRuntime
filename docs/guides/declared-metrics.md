...
| `missing_metrics` | How many were promised as `expected` and never emitted. |
...
parsing a log. `optional` metrics are counted in the audit payload
...
collector = SyncCollector(environment, declared_metrics=environment.spec.metrics)
...
already bound one to a run — it keeps its store binding and its audits.
...
`emitted_metrics`, `missing_metrics`), so a dashboard or a scheduler can read
them from the metric table alone.
