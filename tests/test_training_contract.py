...
def _step(**info):
    return TimeStep({"state": np.array([1])}, np.array([0.0]), np.array([False]), np.array([False]), episode_id=UUID(int=1), info=info)
...
def test_replay_requires_matching_provenance():
    step = _step()
    transition = Transition(step.episode_id, 0, step.observation, {"a": 0}, step.reward, step.observation, step.terminated, step.truncated, provenance=transition_provenance(step))
    assert assert_transition_provenance(transition) is transition
    bad = Transition(step.episode_id, 1, step.observation, {"a": 0}, step.reward, step.observation, step.terminated, step.truncated, provenance=transition_provenance(step))
