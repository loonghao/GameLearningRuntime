...
export function ProcessTrace({
  ...
  useEffect(() => setClosed(false), [selectedEvent?.sequence_id, run?.run_id]);
  ...
  return (
    <section className="process-trace" aria-label="Agent process trace">
      ...
      <div className="trace-overview">
        ...
        {lanes.map((name) => {
          const items = filtered.filter((e) => laneOf(e) === name).slice(-400);
        ...
        <p className="trace-note">
          Receipt-time markers · bars use reported duration only · latest 400
          matching events per lane.
        </p>
      </div>
      ...
      <div className={`trace-body ${selected ? "with-detail" : ""}`}>
        <div className="trace-rows" aria-label="Process steps">
          ...
          {filtered.length > visible && (
            <Button variant="ghost" onClick={() => setVisible((n) => n + 100)}>
              Load earlier steps
            </Button>
          )}
        </div>
        {selected && (
          <aside className="trace-detail" aria-label="Process event details">
